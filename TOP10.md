...
（１）請重新連線至iCHEF網路後重新登入，若網路正常，結帳的時候便會自動下載字軌，
   此時只要再滑動依次結帳按鈕即可。

（２）若上述情況無效，請先連到其他有效網路後，將iCHEF重新下載設定之後登入，
...
設定方式如下：
    ...
    方法一：進入iPAD外帶的點餐頁面，便可調整開關 (內用的點餐介面無法調整)。
...
到ipad「設定」中，點入正在連線的wifi，第一行可以看到IP，通常是192.168.開頭，或10.0.開頭
