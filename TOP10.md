...
（２）紅燈綠燈同時快閃：表示出單機找不到IP網路位置。
請將Wifi分享器重開（白色正方形，有蘋果的圖案），出單機重開，讓它重新分配IP。
...
若網路重新設定時，需重新到工具箱指定印表機：
    ...
    Wifi重新連線＞App重開＞重新登入＞桌位右下角工具箱>印表機＞重新點選印表機
...
到iPad「設定」中，點入正在連線的Wifi，第一行可以看到IP，通常是192．168開頭，或10．0開頭。
...
印表機會自行重置IP位置，完成後請再重複步驟1、2確認IP相同即可。
